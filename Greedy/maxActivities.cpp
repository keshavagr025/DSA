...
int maxActivities(vector<int> start, vector<int> end)
{
    ...
    for (int i = 1; i < start.size(); i++)
    {
        ...
        { // non overlapping condition
            cout << "Selecting activites A" << i << endl;
            count++;
            currEndTime = end[i];
        }
    }
...
bool compare(pair<int, int> p1, pair<int, int> p2){
    return p1.second < p2.second; // acending sorting
}
...
int main()
{
    ...
    cout << "The total count of activites are = " << maxActivities(start, end) << endl;
